...
class FlowControllerTest {
    ...
    @Test
    @DisplayName("GET /flows/{flow-id}는 FlowManager.getSummary() 결과를 200으로 리턴한다")
    ...
    @Test
    @DisplayName("POST /flows/{flow-id}/start는 FlowManager.start()를 호출하고 최신 상태를 반환한다")
    ...
    @Test
    @DisplayName("POST /flows/{flow-id}/stop은 FlowManager.stop()을 호출하고 최신 상태를 반환한다")
    ...
    @Test
    @DisplayName("POST /flows/{flow-id}/restart는 FlowManager.restart()를 호출하고 최신 상태를 반환한다")
    ...
    @Test
    @DisplayName("PATCH /flows/{flow-id}/nodes/{node-id}/config는 FlowConfigService.reconfigure()를 호출하고 204를 리턴한다.")
